(function () {
  ...
  function initMagneticElements() {
    ...
    var selectors = '.btn, .profile-logo, #backToTop, #themeToggle, .project-image-wrap';
    document.querySelectorAll(selectors).forEach(function (item) {
      item.addEventListener('pointermove', function (event) {
        var rect = item.getBoundingClientRect();
        var x = event.clientX - rect.left - rect.width / 2;
        var y = event.clientY - rect.top - rect.height / 2;
        var strength = item.classList.contains('project-image-wrap') ? 0.045 : 0.18;
  ...
  function initCustomCursor() {
    ...
    function render() {
      ...
      dot.style.transform = 'translate3d(' + mouseX.toFixed(2) + 'px, ' + mouseY.toFixed(2) + 'px, 0) translate(-50%, -50%)';
